module.exports = {
    ...
    validate: (tx, ts, legitUser, cb) => {
        ...
        cache.findOne('tokens', {symbol: payload.symbol}, function(err, token) {
            ...
            cache.findOne('accounts', {name: tx.sender}, function(err, account) {
                ...
                const balance = account.tokens?.[payload.symbol] || '0'
    ...
    execute: (tx, ts, cb) => {
        ...
        cache.findOne('accounts', {name: tx.sender}, function(err, senderAccount) {
            ...
            if (!senderAccount.tokens) {
                senderAccount.tokens = {}
            }
            if (!senderAccount.tokens[payload.symbol]) {
                senderAccount.tokens[payload.symbol] = '0'
            }

            // Update sender balance
            const newSenderBalance = (parseFloat(senderAccount.tokens[payload.symbol]) - amount).toString()
            senderAccount.tokens[payload.symbol] = newSenderBalance
            ...
            cache.updateOne('accounts',
                ...
                function(err) {
                    ...
                    cache.findOne('accounts', {name: payload.to}, function(err, recipientAccount) {
                        ...
                        const newRecipientBalance = (parseFloat(recipientAccount.tokens[payload.symbol]) + amount).toString()
                        recipientAccount.tokens[payload.symbol] = newRecipientBalance
