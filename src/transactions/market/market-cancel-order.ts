...
import { calculateTradeValue, toBigInt } from '../../utils/bigint.js';
...
export async function validateTx(data: MarketCancelOrderData, sender: string): Promise<boolean> {
    try {
        ...
        if (
            orderFromCache.status === OrderStatus.FILLED ||
...
export async function processTx(data: MarketCancelOrderData, sender: string, _id: string): Promise<boolean> {
    try {
        ...
        if (unfilledQuantity > toBigInt(0)) {
            ...
            const tokenToReturn = order.side === OrderSide.BUY ? order.quoteAssetSymbol : order.baseAssetSymbol;
