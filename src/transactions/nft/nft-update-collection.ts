...
export async function validateTx(data: NFTUpdateCollectionData, sender: string): Promise<{ valid: boolean; error?: string }> {
    try {
        if (!data.symbol) {
        ...
        if (data.name !== undefined && !validate.string(data.name, 50, 1)) {
            ...
            return { valid: false, error: 'Invalid name length (must be 1-50 characters).' };
        }
        ...
        if (data.description !== undefined && !validate.string(data.description, 1000, 0)) {
            ...
            return { valid: false, error: 'Invalid description length (must be 0-1000 chars).' };
        }
        ...
        if (data.websiteUrl !== undefined && (!validate.string(data.websiteUrl, 2048, 10) || !data.websiteUrl.startsWith('http'))) {
            ...
            return { valid: false, error: 'Invalid websiteUrl format.' };
        }
        ...
        if (data.baseCoverUrl !== undefined && (!validate.string(data.baseCoverUrl, 2048, 10) || !data.baseCoverUrl.startsWith('http'))) {
            ...
            return { valid: false, error: 'Invalid baseCoverUrl format.' };
        }
        ...
        if (data.royaltyBps !== undefined) {
            if (!validate.integer(data.royaltyBps, true, false, 2500, 0)) {
                ...
                return { valid: false, error: 'Invalid royaltyBps format.' };
            }
        }
        ...
        if (collection.creator !== sender) {
            ...
            return { valid: false, error: 'Sender is not the creator of the collection.' };
        }
...
export async function processTx(data: NFTUpdateCollectionData, sender: string, _id: string): Promise<{ valid: boolean; error?: string }> {
    try {
        ...
        if (!collection || collection.creator !== sender) {
            ...
            return { valid: false, error: 'Collection not found or sender is not creator.' };
        }
        ...
        if (!updateSuccess) {
            ...
            return { valid: false, error: 'Failed to update collection.' };
        }
        ...
        await logEvent('nft', 'collection_updated', sender, {
            ...
            previousData: {
                ...
                baseCoverUrl: collection.baseCoverUrl,
