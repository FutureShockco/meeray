...
export async function validateTx(data: NftAcceptOfferData, sender: string): Promise<{ valid: boolean; error?: string }> {
    try {
        ...
        if (!offer || offer.status !== 'ACTIVE') {
            ...
            return { valid: false, error: 'offer not found or not active' };
        }
        if (offer.offerBy === sender) {
            logger.warn('[nft-accept-offer] Cannot accept own offer.');
            return { valid: false, error: 'cannot accept own offer' };
        }
        ...
        if (offer.targetType === 'NFT') {
            ...
            if (!nft || nft.owner !== sender) {
                ...
                return { valid: false, error: 'NFT not found or not owned by sender' };
            }
            ...
            if (activeListing) {
                ...
                return { valid: false, error: 'cannot accept offer on listed NFT' };
            }
            ...
            if (!data.nftInstanceId || !validate.string(data.nftInstanceId, 256, 3)) {
                ...
                return { valid: false, error: 'nftInstanceId required for collection/trait offers' };
            }
            ...
            if (offer.targetType === 'COLLECTION') {
                ...
                if (nftCollectionSymbol !== offer.targetId) {
                    ...
                    return { valid: false, error: 'NFT not from collection' };
                }
            }
...
export async function processTx(data: NftAcceptOfferData, sender: string, _id: string): Promise<{ valid: boolean; error?: string }> {
    try {
        ...
        if (royaltyAmount > 0n && collection.creator && collection.creator !== sender) {
            if (!(await adjustUserBalance(collection.creator, paymentToken.symbol, royaltyAmount))) {
                ...
                return { valid: false, error: 'failed to pay royalty to creator' };
            }
        }
