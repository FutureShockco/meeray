...
export async function processTx(data: CloseAuctionData, sender: string): Promise<boolean> {
  try {
    ...
    if (!winningBid) {
      ...
      await cache.updateOnePromise('nftListings', { _id: data.listingId }, { $set: { status: 'ended', endedAt: new Date().toISOString(), endedBy: sender } });

      return true;
    }

    // Process winning bid (similar to accept bid logic)
    const collection = (await cache.findOnePromise('nftCollections', { _id: listing.collectionId })) as CachedNftCollectionForTransfer & {
      royaltyBps?: number;
    };
    ...
    const updateNftOwnerSuccess = await cache.updateOnePromise('nfts', { _id: fullInstanceId, owner: listing.seller }, { $set: { owner: winningBid.bidder, lastTransferredAt: new Date().toISOString() } });
    ...
    const updateWinningBidSuccess = await cache.updateOnePromise('nftBids', { _id: winningBid._id }, { $set: { status: 'won', wonAt: new Date().toISOString() } });
