...
export async function validateTx(data: NftBatchPayload, sender: string): Promise<boolean> {
  try {
    ...
    for (let i = 0; i < data.operations.length; i++) {
      ...
      try {
        switch (op.operation) {
          case 'LIST': isValid = await validateListTx(op.data, sender); break;
          case 'DELIST': isValid = await validateDelistTx(op.data, sender); break;
          case 'BUY':
          case 'BID': isValid = await validateBuyTx(op.data, sender); break;
          case 'TRANSFER': isValid = await validateTransferTx(op.data, sender); break;
        }
      } catch (error) {
