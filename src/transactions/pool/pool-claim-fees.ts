...
export async function processTx(data: PoolClaimFeesData, sender: string, transactionId: string): Promise<boolean> {
  try {
    ...
    // Log event
    await logEvent('defi', 'fees_claimed', sender, {
      poolId: data.poolId,
    ...
    logger.debug(`[pool-claim-fees] User ${sender} successfully claimed ${result.feesClaimedA} ${pool.tokenA_symbol} and ${result.feesClaimedB} ${pool.tokenB_symbol} from pool ${data.poolId}`);
