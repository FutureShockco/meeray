...
export const p2p = {
    ...
    handleBlockConfRound: (ws: EnhancedWebSocket, message: any): void => {
        logger.debug(`Received BLOCK_CONF_ROUND from ${message.s?.n}: round=${message.d?.r}, block hash=${message.d?.b?.hash} (old P2P handler)`);
        if (p2p.recovering) return;
        ...
        if (index !== -1) {
            ...
            if (currentPeerCount < minPeersForConsensus) {
                if (now - p2p.lastEmergencyDiscovery > emergencyCooldown) {
                    ...
                    // Exit sync mode if we're in it - can't maintain sync coordination with insufficient peers
    ...
    broadcast: (data: any): void => {
        logger.debug(`P2P broadcast: Sending to ${p2p.sockets.length} connected sockets. Message type: ${data.t}`);
