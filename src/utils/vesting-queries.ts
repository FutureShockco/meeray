...
import { VestingState, TokenDistributionRecipient } from '../transactions/launchpad/launchpad-interfaces.js';
...
export async function getVestingStatus(
  ...
  const allocation = launchpad.tokenomicsSnapshot.allocations.find(
    (a: any) => a.recipient === allocationType
  );
 ...
 */
export async function getUserVestingStates(
  userId: string,
  currentTimestamp: string
): Promise<VestingStatusResult[]> {
  ...
  for (const state of vestingStates) {
    const status = await getVestingStatus(
      state.userId,
      state.launchpadId,
 ...
 */
export async function getLaunchpadVestingStates(
  launchpadId: string,
  currentTimestamp: string
): Promise<VestingStatusResult[]> {
