...
export const skipBridgeOperationsUntilBlock: number = process.env.SKIP_BRIDGE_OPERATIONS_UNTIL_BLOCK
    ? parseInt(process.env.SKIP_BRIDGE_OPERATIONS_UNTIL_BLOCK)
    : 0;
...
export const steemApiUrls: string[] = process.env.STEEM_API
    ? process.env.STEEM_API.split(',')
          .map(s => s.trim())
          .filter(Boolean)
    : [];
...
export const peers: string[] = process.env.PEERS
