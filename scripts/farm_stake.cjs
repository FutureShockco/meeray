...
async function main() {
    ...
    const customJsonOperation = [
        'custom_json',
        {
            ...
            json: JSON.stringify({
                contract: 'farm_stake',
                payload: farmStakeData
            })
        }
    ];

    console.log('Attempting to broadcast Farm Stake operation:');
    ...
    try {
        ...
        console.log('Farm Stake operation broadcasted successfully!');
        console.log('Transaction ID:', result.id);
        console.log('Block Number:', result.block_num);
    } catch (error) {
        console.error('Error broadcasting Farm Stake operation:', error.message);
