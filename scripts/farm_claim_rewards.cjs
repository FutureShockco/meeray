...
async function main() {
    ...
    const customJsonOperation = [
        'custom_json',
        {
            ...
            id: 'sidechain',
            json: JSON.stringify({
                contract: 'farm_claim_rewards',
                payload: farmClaimData
            })
        }
    ];

    console.log('Attempting to broadcast Farm Claim Rewards operation:');
    ...
    try {
        ...
        console.log('Farm Claim Rewards operation broadcasted successfully!');
        console.log('Transaction ID:', result.id);
        console.log('Block Number:', result.block_num);
    } catch (error) {
        console.error('Error broadcasting Farm Claim Rewards operation:', error.message);
