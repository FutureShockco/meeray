...
async function main() {
    ...
    try {
        await sendCustomJson(client, sscId, 'launchpad_refund_presale', payload, username, privateKey);
        console.log('Refund presale executed for', launchpadId);
    } catch (err) {
        console.error('Failed to refund presale:', err.message);
    }
}
